...
export default function HistoryChart({ runs }: { runs: Run[] }) {
  ...
  return (
    <div className="space-y-4">
      {runs.length === 0 ? (
        <p className="text-slate-500 text-center py-8">עדיין אין ריצות מתועדות</p>
      ) : (
        <>
          <div className="bg-slate-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="מרחק" stroke="#60a5fa" strokeWidth={2} dot={{ r: 3, fill: '#60a5fa' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={chartData}>
                ...
                <Tooltip contentStyle={darkTooltipStyle} formatter={(v) => typeof v === 'number' ? formatPace(v) : v} />
                <Line type="monotone" dataKey="קצב" stroke="#4ade80" strokeWidth={2} dot={{ r: 3, fill: '#4ade80' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
